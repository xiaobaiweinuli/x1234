...
object RootManager {
    ...
    suspend fun exec(vararg cmds: String): List<String> = withContext(Dispatchers.IO) {
        ...
        LogManager.d(TAG, "执行 Root 命令: $cmdString")
        
        // 确保已获得 Root 权限
        if (!isGranted) {
            val granted = requestRoot()
            if (!granted) {
                throw RuntimeException("Root 权限未授权")
            }
        }
        ...
        if (exitCode != 0 && output.isEmpty()) {
            val errorMsg = error.ifEmpty { "未知错误" }
            LogManager.e(TAG, "Root 命令失败: $errorMsg")
            throw RuntimeException("Root 命令失败: $errorMsg")
        }
        
        LogManager.d(TAG, "Root 命令执行成功")
        output.lines().filter { it.isNotBlank() }
    }

    /**
     * 以 root 身份执行 shell 命令（静默模式，不抛异常）
     * 使用全局命名空间方式，支持访问私有目录
     * @return 命令输出列表，失败返回空列表
     */
    suspend fun execSilent(vararg cmds: String): List<String> = withContext(Dispatchers.IO) {
        try {
            val cmdString = cmds.joinToString(" ")
            LogManager.d(TAG, "执行 Root 命令 (静默): $cmdString")
            ...
            if (exitCode != 0 && output.isEmpty()) {
                LogManager.w(TAG, "Root 命令失败: ${error.ifEmpty { "未知错误" }}")
                return@withContext emptyList()
            }
